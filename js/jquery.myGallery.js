...
function myGallery() {
    ...
    function userEventsGallery(element) {
        ...
        this.handleGestureStartNoDelay = function(e) {
            ...
            if ((new Date().getTime()) - g_timeLastTouchStart < 400) {
                return;
            }
            g_openNoDelay = true;
    ...
    function checkPluginParameters() {
        ...
        if (gO.userID.toUpperCase() == 'CBRISBOIS@GMAIL.COM' || gO.userID == '111186676244625461692') {
            if (gO.blackList == '' || gO.blackList.toUpperCase() == 'SCRAPBOOK|PROFIL') {
                gO.blackList = 'profil|scrapbook|forhomepage';
            }
        }

        if (gO.blackList != '') {
            g_blackList = gO.blackList.toUpperCase().split('|');
        }
        if (gO.whiteList != '') {
            g_whiteList = gO.whiteList.toUpperCase().split('|');
        }
        if (gO.albumList != '') {
            g_albumList = gO.albumList.toUpperCase().split('|');
        }
    ...
    */
    ...
    function RetrieveCurWidth() {
        ...
        if (gO.thumbnailSizeSM > 0 && vpW < gO.thumbnailSizeSM) {
            return 'xs';
        }
        if (gO.thumbnailSizeME > 0 && vpW < gO.thumbnailSizeME) {
            return 'sm';
        }
        if (gO.thumbnailSizeLA > 0 && vpW < gO.thumbnailSizeLA) {
            return 'me';
        }
        if (gO.thumbnailSizeXL > 0 && vpW < gO.thumbnailSizeXL) {
            return 'la';
        }
    ...
    */
    ...
    function retrieveThumbnailSizes() {
        ...
        var desc = '';
        if (gO.thumbnailLabel.displayDescription) {
            desc = 'd';
        }
        ...
        if (g_custGlobals.oldBorderColor == '' || g_custGlobals.oldBorderColor == null || g_custGlobals.oldBorderColor == undefined) {
            g_custGlobals.oldBorderColor = '#000';
        }
    ...
    function renderGallery2(albumIdx, pageNumber, onComplete) {
        if (albumIdx == -1 || gI[albumIdx] == undefined) {
            return;
        }
    ...
    function thumbnailBuild(item, idx, eltOpacity, foundDesc) {
        ...
        if (item.kind == 'album') {
            ...
            // IMAGE
            if (gO.thumbnailLabel.display == true) {
                if (foundDesc && sDesc.length == 0 && gO.thumbnailLabel.position == 'onBottom') {
                    sDesc = '&nbsp;';
                }
        ...
        if (checkImageSize) {
            ...
            gi_imgLoad.on('always', function(instance) {
                ...
                if (item == undefined || jQuery(instance.images[0].img).attr('src') == g_emptyGif) {
                    return;
                } // also fired for blank image --> ignore
                var b = false;
    ...
    function setThumbnailSize($elt, item) {
        ...
        if (SettingsGetTnWidth() == 'auto') {
            ...
            if (item.thumbImg().width > 0) {
                ...
                $elt.find('.imgContainer').width(SettingsGetTnHeight() * ratio).css({
                    overflow: 'hidden'
                });
                if (gO.thumbnailLabel.position == 'onBottom') {
    ...
    function thumbnailPositionContent($e, item) {
        ...
        switch (gO.thumbnailLabel.position) {
            case 'onBottom':
                ...
                if (SettingsGetTnHeight() == 'auto') {
                    $e.find('.labelImageTitle').css({
                        'white-space': 'normal'
                    }); // line break
                    $e.find('.labelFolderTitle').css({
                        'white-space': 'normal'
                    });
                    $e.find('.labelDescription').css({
                        'white-space': 'normal'
                    });
                } else {
                    $e.find('.labelImageTitle').css({
            ...
            switch (g_tnHE[j].name) {
                ...
                case 'imageFlipHorizontal':
                    switch (gO.thumbnailLabel.position) {
                        case 'overImageOnTop':
                            $e.find('.labelImage').css({
                                top: -g_tn.imgcBorderHeight / 2,
                                bottom: g_tn.imgcBorderWidth / 2,
                                left: 0,
                                right: 0
                            });
                            break;
    ...
    function TnAni($e, n, anime, item, eltClass) {
            ...
            for (var i = 0; i < transform.length; i++) {
                ...
                if (anime[tf] !== undefined) {
                    var from = {
                        ...
                        tf: tf,
                        eltClass: eltClass,
                        to: parseInt(anime[tf])
                    };
                    var to = {
                        v: parseInt(anime[tf])
                    };
                    if (g_tnHE[n].delay > 0) {
                        jQuery(from).delay(g_tnHE[n].delay).animate(to, {
                            duration: g_tnHE[n].duration,
                            easing: g_tnHE[n].easing,
                            queue: false,
        ...
        if (g_tnHE[n].delay > 0) {
            ...
            if (g_aengine == 'transition') {
                ...
                $e.delay(g_tnHE[n].delay)[g_aengine](anime, {
                    duration: g_tnHE[n].duration,
                    easing: g_tnHE[n].easing,
                    queue: false
                });
        ...
        try {
            for (j = 0; j < g_tnHE.length; j++) {
                switch (g_tnHE[j].name) {
                    ...
                    case 'scaleLabelOverImage':
                        TnAni($e.find('.labelImage'), j, {
                            scale: 100 / dscale,
                            opacity: 1
                        }, item, 'labelImage0');
                        TnAni($e.find('.imgContainer'), j, {
                            scale: 50 / dscale
                        }, item, 'imgContainer0');
                        break;
                    ...
                    case 'rotateCornerBL':
                        var r = (g_aengine == 'transition' ? {
                            rotate: '0deg'
                        } : {
                            rotateZ: '0'
                        });
                        TnAni($e.find('.labelImage'), j, r, item, 'labelImage0');
                        r = (g_aengine == 'transition' ? {
                            rotate: '90deg'
                        } : {
                            rotateZ: '90'
                        });
                    ...
                    case 'labelAppear':
                        if (g_aengine == 'velocity') {
                            TnAni($e.find('.labelImage'), j, {
                                backgroundColorRed: g_custGlobals.oldLabelRed,
                                backgroundColorGreen: g_custGlobals.oldLabelGreen,
                                backgroundColorBlue: g_custGlobals.oldLabelBlue,
                        ...
                        TnAni($e.find('.labelImageTitle'), j, {
                            opacity: 1
                        }, item);
                        TnAni($e.find('.labelFolderTitle'), j, {
                            opacity: 1
                        }, item);
                        TnAni($e.find('.labelDescription'), j, {
                            opacity: 1
                        }, item);
                        break;
                    ...
                    case 'borderLighter':
                        if (g_aengine == 'velocity') {
                            ...
                            TnAni($e, j, {
                                borderColor: lighterColor(g_custGlobals.oldBorderColor, 0.5)
                            }, item);
                        }
                        break;
                    ...
                    case 'borderDarker':
                        if (g_aengine == 'velocity') {
                            ...
                            TnAni($e, j, {
                                borderColor: darkerColor(g_custGlobals.oldBorderColor, 0.5)
                            }, item);
                        }
                        break;
    ...
    function TnAniO($e, n, anime, item, eltClass) {
            ...
            for (var i = 0; i < transform.length; i++) {
                ...
                if (anime[tf] !== undefined) {
                    ...
                    if (g_tnHE[n].delayBack > 0) {
                        jQuery(from).delay(g_tnHE[n].delayBack).animate(to, {
                            duration: g_tnHE[n].durationBack,
                            easing: g_tnHE[n].easingBack,
                            queue: false,
                            step: function(currentValue) {
                                item.eltTransform[this.eltClass][this.tf] = currentValue;
                                SetCSSTransform(this.item, this.eltClass);
        ...
        if (g_tnHE[n].delay > 0) {
            if (g_aengine == 'transition') {
                ...
                $e.delay(g_tnHE[n].delayBack)[g_aengine](anime, {
                    duration: g_tnHE[n].durationBack,
                    easing: g_tnHE[n].easingBack,
                    queue: false
                });
        ...
        try {
            for (j = 0; j < g_tnHE.length; j++) {
                switch (g_tnHE[j].name) {
                    ...
                    case 'scaleLabelOverImage':
                        TnAniO($e.find('.labelImage'), j, {
                            opacity: 0,
                            scale: 50 / dscale
                        }, item, 'labelImage0');
                        TnAniO($e.find('.imgContainer'), j, {
                            scale: 100 / dscale
                        }, item, 'imgContainer0');
                        break;
                    ...
                    case 'labelAppear75':
                        ...
                        TnAniO($e.find('.labelImageTitle'), j, {
                            opacity: 0
                        });
                        TnAniO($e.find('.labelFolderTitle'), j, {
                            opacity: 0
                        });
                        TnAniO($e.find('.labelDescription'), j, {
                            opacity: 0
                        });
                        break;
                    ...
                    case 'labelOpacity50':
                        TnAniO($e.find('.labelImage'), j, {
                            opacity: g_custGlobals.oldLabelOpacity
                        });
                        break;
                    ...
                    case 'borderDarker':
                        if (g_aengine == 'velocity') {
                            ...
                            TnAniO($e, j, {
                                borderColor: g_custGlobals.oldBorderColor
                            });
                        }
                        break;
    ...
    function OpenInternalViewer(imageIdx) {
        ...
        $closeB.on("touchstart click", function(e) {
            e.stopPropagation();
            if ((new Date().getTime()) - g_timeImgChanged < 400) {
                return;
            }
        ...
        $gE.conVwTb.find('.nextButton').on("touchstart click", function(e) {
            e.stopPropagation();
            DisplayNextImagePart1();
        });
        $gE.conVwTb.find('.previousButton').on("touchstart click", function(e) {
            e.stopPropagation();
            DisplayPreviousImage();
        });
        $gE.vwContent.find('.contentAreaNext').on("touchstart click", function(e) {
            e.stopPropagation();
            DisplayNextImagePart1();
        });
        $gE.vwContent.find('.contentAreaPrevious').on("touchstart click", function(e) {
            e.stopPropagation();
            DisplayPreviousImage();
        });
        ...
        $gE.conVw.find('.image').attr('draggable', 'false').attr('unselectable', 'on').css({
        ...
        if (gO.slideshowAutoStart) {
            ...
            DisplayNextImage();
            g_playSlideshowTimerID = window.setInterval(function() {
                DisplayNextImage();
            }, g_slideshowDelay);
        }
    ...
    function DisplayNextImage() {
        ...
        if ((new Date().getTime()) - g_timeImgChanged < 300) {
            return;
        }
        var l = gI.length;
    ...
    function DisplayInternalViewer(imageIdx, displayType) {
        ...
        if (displayType == '') {
            ...
            $gE.vwImgC.css({
                ...
                left: 0,
                visibility: 'visible'
            }).attr('src', g_emptyGif).attr('src', gI[imageIdx].responsiveURL());
    ...
    */
    ...
    function DisplayInternalViewerComplete(imageIdx, displayType) {
        ...
        $gE.vwImgN.css({
            ...
            left: 0,
            visibility: 'hidden'
        }).attr('src', g_emptyGif).attr('src', gI[GetNextImageIdx(imageIdx)].responsiveURL());
        //$gE.vwImgN.css({'opacity':0, 'right':'0', 'left':'0', visibility: 'hidden'}).attr('src',gI[GetNextImageIdx(imageIdx)].responsiveURL());
        $gE.vwImgP.css({
            opacity: 0,
            left: 0,
            visibility: 'hidden'
        }).attr('src', g_emptyGif).attr('src', gI[GetPreviousImageIdx(imageIdx)].responsiveURL());
    ...
    function ResizeInternalViewer($imgElt) {
        ...
        window.clearInterval(g_viewerResizeTimerID);
        g_viewerResizeTimerID = window.setInterval(function() {
            ResizeInternalViewer($gE.vwImgC)
        }, 200);
        ...
        if (gO.viewerToolbar.style == 'innerImage') {
            $gE.conVwTb.find('.toolbar').css({
                'max-width': $gE.vwImgC.width()
            });
        }
        ...
        $gE.conVwTb.css({
            height: $gE.conVwTb.find('.toolbar').outerHeight(true)
        });
        $gE.vwContent.children('img').css({
            'max-width': (w - tH),
            'max-height': (h - tV)
        });
    }
    ...
    function SetColorScheme(element) {
        ...
        if (cs.navigationbar.border !== undefined) {
            s += s1 + '.myGalleryNavigationbar { border:' + cs.navigationbar.border + ' !important; }' + '\n';
        }
        if (cs.navigationbar.borderTop !== undefined) {
            s += s1 + '.myGalleryNavigationbar { border-top:' + cs.navigationbar.borderTop + ' !important; }' + '\n';
        }
        if (cs.navigationbar.borderBottom !== undefined) {
            s += s1 + '.myGalleryNavigationbar { border-bottom:' + cs.navigationbar.borderBottom + ' !important; }' + '\n';
        }
        if (cs.navigationbar.borderRight !== undefined) {
            s += s1 + '.myGalleryNavigationbar { border-right:' + cs.navigationbar.borderRight + ' !important; }' + '\n';
        }
        if (cs.navigationbar.borderLeft !== undefined) {
            s += s1 + '.myGalleryNavigationbar { border-left:' + cs.navigationbar.borderLeft + ' !important; }' + '\n';
        }
    ...
    function nanoConsoleLog(msg) {
        if (window.console) {
            console.log('myGallery: ' + msg + ' [' + g_baseEltID + ']');
        }
    };
